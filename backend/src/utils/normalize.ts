...
export function normalizeCandles(candles: Candle[]): Candle[] {
  return candles.map((candle) => ({
    ...
    // Fix importante: timestamp do MT5 é Unix seconds, mas new Date espera milissegundos
    timestamp: typeof candle.timestamp === 'number' && candle.timestamp < 1e11 
